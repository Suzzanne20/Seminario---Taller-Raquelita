...
@section('content')
    <div class="container">
        <div class="md-card">
            <h2 class="md-title">Editar Orden de Compra N° {{ $ordenes->id }}</h2>
            ...
            <form action="{{ route('ordenes_compras.update', $ordenes->id) }}" method="POST" id="formOrdenCompra" novalidate>
                @csrf
                @method('PUT') {{-- Usa el método PUT para la actualización --}}

                <div class="row g-3">
                    {{-- Campo de SOLO LECTURA para el número de orden --}}
                    <div class="col-md-6">
                        <label for="numero_orden" class="form-label">Número de Orden</label>
                        {{-- Se mantiene como hidden para ser enviado al controlador si es necesario para validación,
                           pero el valor visible es de solo lectura. --}}
                        <input id="numero_orden_display" type="text" class="form-control"
                               value="{{ $ordenes->id }}" readonly style="background:#f4f4f4;">
                        <input type="hidden" name="numero_orden" value="{{ $ordenes->id }}">
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input id="fecha_orden" name="fecha_orden" type="date"
                               ...
                               value="{{ old('fecha_orden', $ordenes->fecha_orden) }}" required>
                    </div>
                </div>
                ...
                <div class="row g-3 mt-1">
                    <div class="col-md-6">
                        ...
                        <input id="fecha_entrega_esperada" name="fecha_entrega_esperada" type="date"
                               ...
                               value="{{ old('fecha_entrega_esperada', $ordenes->fecha_entrega_esperada) }}">
                    </div>
                        ...
                        <select id="proveedor_id" name="proveedor_id"
                            ...
                            @foreach($proveedores as $proveedor)
                                <option value="{{ $proveedor->id }}"
                                    {{ old('proveedor_id', $ordenes->proveedor_id) == $proveedor->id ? 'selected' : '' }}>
                                    {{ $proveedor->nombre }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row g-3 mt-1">
                    <div class="col-md-4">
                        ...
                        <select id="estado" name="estado" class="form-select @error('estado') is-invalid @enderror" required>
                            @foreach(['pendiente', 'aprobada', 'recibida', 'cancelada'] as $estado)
                                <option value="{{ $estado }}"
                                    {{ old('estado', $ordenes->estado) == $estado ? 'selected' : '' }}>
                                    {{ ucfirst($estado) }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-8">
                        ...
                        <textarea id="observaciones" name="observaciones" rows="3"
                                  ...
                                  placeholder="Notas adicionales sobre la orden…">{{ old('observaciones', $ordenes->observaciones) }}</textarea>
                    </div>
                </div>
                ...
                <div class="section-divider">
                    ...
                    <div id="detallesContainer">
                        ...
                        @foreach(old('detalles', $ordenes->detalles) as $index => $detalle)
                            @php
                                // Obtener los datos para el JS si estamos re-renderizando un error de old()
                                $detalleData = is_array($detalle) ? $detalle : $detalle->toArray();
                                $currentInsumoId = $detalleData['insumo_id'];
                                $currentCantidad = $detalleData['cantidad'];
                                $currentPrecioUnitario = $detalleData['precio_unitario'];
                                $currentSubtotal = $detalleData['subtotal'];
                                $currentPrecioInsumo = $insumos->firstWhere('id', $currentInsumoId)->costo ?? 0;
                            @endphp
                            <div class="detalle-row" data-index="{{ $index }}">
                                <div class="row g-2 align-items-end">
                                    <div class="col-md-5">
                                        <label class="form-label">Insumo</label>
                                        <select name="detalles[{{ $index }}][insumo_id]" class="form-select detalle-insumo" required onchange="actualizarPrecio(this)">
                                            <option value="">— Seleccione insumo —</option>
                                            @foreach($insumos as $insumo)
                                                <option value="{{ $insumo->id }}" data-precio="{{ $insumo->costo }}"
                                                    {{ $currentInsumoId == $insumo->id ? 'selected' : '' }}>
                                                    {{ $insumo->nombre }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-md-2">
                                        ...
                                        <input type="number" name="detalles[{{ $index }}][cantidad]" class="form-control detalle-cantidad"
                                               step="0.01" min="0.01" value="{{ number_format($currentCantidad, 2, '.', '') }}" required onchange="calcularSubtotal(this)">
                                    </div>
                                    <div class="col-md-2">
                                        <label class="form-label">Precio Unit.</label>
                                        <input type="number" name="detalles[{{ $index }}][precio_unitario]" class="form-control detalle-precio"
                                               step="0.01" min="0" value="{{ number_format($currentPrecioUnitario, 2, '.', '') }}" required onchange="calcularSubtotal(this)">
                                    </div>
                                    <div class="col-md-2">
                                        <label class="form-label">Subtotal</label>
                                        <input type="text" class="form-control detalle-subtotal-display" readonly value="Q {{ number_format($currentSubtotal, 2) }}" style="font-weight:600;">
                                        <input type="hidden" name="detalles[{{ $index }}][subtotal]" class="detalle-subtotal-value" value="{{ number_format($currentSubtotal, 2, '.', '') }}">
                                    </div>
                                    <div class="col-md-1">
                ...
                <div class="total-box mt-3">
                    ...
                    <div id="totalGeneral">Q {{ number_format(old('total', $ordenes->total), 2) }}</div>
                    <input type="hidden" name="total" id="inputTotal" value="{{ old('total', $ordenes->total) }}">
                </div>
    ...
    <script>
        ...
        // Inicializar el contador con el número de detalles existentes + 1, para no sobreescribir índices.
        let detalleCounter = {{ count(old('detalles', $ordenes->detalles)) }};
        ...
        function agregarDetalle() {
            ...
            div.innerHTML = `
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-md-2">
                        <label class="form-label">Subtotal</label>
                        <input type="text" class="form-control detalle-subtotal-display" readonly value="Q 0.00" style="font-weight:600;">
        ...
        function calcularSubtotal(input) {
            ...
            row.querySelector('.detalle-subtotal-display').value = `Q ${subtotal.toFixed(2)}`;
        ...
        // NO agregamos detalle inicial automáticamente, ya que se cargan los detalles existentes.
        // Asegurarse de que el total se calcule al cargar la página si hay detalles.
