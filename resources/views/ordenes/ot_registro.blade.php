            ...
            <form action="{{ route('ordenes.store') }}" method="POST" novalidate>
                ...
                <div class="row g-4">
                    ...
                    @push('scripts')
                        <script>
                            document.addEventListener('DOMContentLoaded', function(){
                                const mark  = document.getElementById('mark-full');
                                const unmark= document.getElementById('unmark-all');
                                if(mark){
...
@push('scripts')
    <script>
      ...
      <div class="row align-items-center i-row" data-i="${i}">
        <div class="col-md-6 mb-2 mb-md-0" style="position:relative;">
          <input type="text"
                 class="form-control insumo-search"
                 placeholder="Buscar insumo...">
          <input type="hidden" name="insumos[${i}][id]" class="insumo-id">
    ...
    `;
        ...
        function recalc(){
            ...
            document.querySelectorAll('#insumos-container .i-row').forEach(row=>{
                ...
                const price = parseFloat(row.querySelector('.insumo-precio').value || 0);
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            cont?.addEventListener('input', e => {
                ...
                if(e.target.classList.contains('insumo-search')){
                    const row = e.target.closest('.i-row');
          ...
          <div class="insumo-option"
            ...
            ${i.nombre} — Q${Number(i.precio || 0).toFixed(2)}
          </div>
        `).join('');
            ...
            cont?.addEventListener('click', e => {
                ...
                if (opt){
                    ...
                    row.querySelector('.insumo-precio').value  = opt.dataset.precio;
            ...
            document.getElementById('costo_mo')?.addEventListener('input', recalc);

            // Ocultar dropdowns al hacer clic fuera
            document.addEventListener('click', e => {
                if(!e.target.closest('.i-row')){
            ...
            (function(){
                ...
                const placa = document.getElementById('vehiculo_placa');
                function toggleReq(){ if(placa) placa.required = !cot.value; }
                cot?.addEventListener('change', toggleReq);
                toggleReq();
            })();

            // === TomSelect cliente y vehículo (igual que antes) ===
            if(window.TomSelect){
