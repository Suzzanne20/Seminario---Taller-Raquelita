...
<div class="container py-4">
  <h1 class="text-center mb-4" style="color:#C24242;">Editar Orden #{{ $orden->id }}</h1>
        ...
        <form action="{{ route('ordenes.update', $orden->id) }}" method="POST" novalidate>
          ...
          <div class="row g-3">
            <div class="col-md-6">
              ...
              <select name="vehiculo_placa" class="form-select" required>
                @foreach($vehiculos as $v)
                  <option value="{{ $v->placa }}" @selected(old('vehiculo_placa', $orden->vehiculo_placa)==$v->placa)>
            ...
            <div class="col-md-4">
              ...
              <input type="number" name="kilometraje" class="form-control" value="{{ old('kilometraje', $orden->kilometraje) }}" required>
            </div>

            <div class="col-md-4">
              <label class="form-label fw-semibold">Próximo servicio (km)</label>
              <input type="number" name="proximo_servicio" class="form-control" value="{{ old('proximo_servicio', $orden->proximo_servicio) }}">
            </div>

            <div class="col-md-4">
              <label class="form-label fw-semibold">Costo mano de obra (Q)</label>
              <input type="number" step="0.01" name="costo_mo" class="form-control" value="{{ old('costo_mo', $orden->costo_mo) }}">
            </div>

            <div class="col-12">
              <label class="form-label fw-semibold">Descripción / Falla</label>
              <textarea name="descripcion" rows="3" class="form-control">{{ old('descripcion', $orden->descripcion) }}</textarea>
            </div>
          </div>

          <div class="d-flex gap-2 mt-4">
            <button class="btn btn-theme"><i class="bi bi-floppy"></i> Guardar cambios</button>
